...
export default function EmailNotificationPage() {
  ...
  return <div className="mx-auto max-w-5xl">
    ...
    {details && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-4"><div className="max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-lift"><div className="flex justify-between gap-4"><div><h2 className="text-lg font-bold">{details.subject}</h2><p className="mt-1 text-xs text-slate-500">{details.recipient_count} recipients · {formatDate(details.sent_at || details.created_at)}</p></div><button onClick={() => setDetails(null)} className="text-slate-500">Close</button></div><pre className="mt-5 whitespace-pre-wrap rounded-xl bg-slate-50 p-4 font-sans text-sm leading-6">{details.message_body}</pre><div className="mt-5 divide-y divide-slate-100 rounded-xl border border-slate-200">{details.recipients.map(recipient => <div key={recipient.user_id} className="flex items-center justify-between gap-3 p-3"><div><p className="text-sm font-semibold">{recipient.name}</p><p className="text-xs text-slate-500">{recipient.email}</p>{recipient.error_message && <p className="mt-1 text-xs text-red-600">{recipient.error_message}</p>}</div><DeliveryStatus value={recipient.status} /></div>)}</div></div></div>}
  </div>
}
